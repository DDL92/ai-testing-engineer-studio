...
import { buildDeliveryReportDocuments } from './clientReportingRules';
...
function main(): void {
  ...
  if (!id) {
    exitWithError('Missing required --id client_id argument. Example: npm run client:delivery-report -- --id demo-retainer-client');
  }
  ...
  const documents = buildDeliveryReportDocuments(input);
  ...
  console.log(`Client delivery report generated: ${path.relative(process.cwd(), outputDir)}`);
  for (const document of documents) {
    console.log(`- ${path.relative(process.cwd(), path.join(outputDir, document.fileName))}`);
  }
  console.log(`Client: ${client.companyName}`);
  console.log('No evidence was invented. No integrations, scraping, browser automation, APIs, payments, CRM, or client systems were used.');
  console.log('Human approval is required before sharing any report.');
}
