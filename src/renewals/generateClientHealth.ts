...
import { buildClientHealthDocuments, buildRenewalClientRecord, source } from './renewalRules';
import { RenewalClientSources, RenewalSource } from './types';
...
function main(): void {
  ...
  if (!id) {
    exitWithError('Missing required --id client_id argument. Example: npm run renewal:review -- --id demo-retainer-client');
  }
  ...
  const record = buildRenewalClientRecord(client, detectSources(client.id));
  const documents = buildClientHealthDocuments(record);
  ...
  console.log(`Client renewal review generated: ${path.relative(process.cwd(), outputRoot)}`);
  for (const document of documents) {
