...
function renderLegacyDashboardMarkdown(data: ReturnType<typeof buildPwaDashboardData>): string {
  ...
  return [
    ...
    renderList([
      ...
      `Revenue Learning Best Offer: ${data.revenueLearning.bestOffer}`,
      ...
      `Studio Doctor Health: ${data.studioDoctor.studioHealth}`,
      `Studio Doctor Command Health: ${data.studioDoctor.commandHealth}`,
      `Studio Doctor Runtime Health: ${data.studioDoctor.runtimeHealth}`,
      `Studio Doctor Evidence Health: ${data.studioDoctor.evidenceHealth}`,
...
function renderLegacyDashboardHtml(data: ReturnType<typeof buildPwaDashboardData>): string {
  ...
  const rows = [
    ...
    ['Next Delivery Action', data.clientDelivery.nextDeliveryAction],
    ['Automation Status', data.automationDelivery.automationStatus],
    ['Critical Flows', String(data.automationDelivery.criticalFlowCount)],
    ['Delivery Assets', String(data.automationDelivery.deliveryAssets)],
    ['Framework Status', data.automationDelivery.frameworkStatus],
    ['Client Handoff Status', data.automationDelivery.clientHandoffStatus],
    ['Delivery Assets Status', data.deliveryAssets.deliveryAssetsStatus],
    ['Executive Report Status', data.deliveryAssets.executiveReportStatus],
    ...
    ['Maintenance Status', data.retainerOperations.maintenanceStatus],
    ['Renewal Status', data.retainerOperations.renewalStatus],
    ['Expansion Opportunities', String(data.retainerOperations.expansionOpportunities)],
    ['Revenue Learning Status', data.revenueLearning.revenueLearningStatus],
    ['Outcomes Recorded', String(data.revenueLearning.outcomesRecorded)],
    ['Best Channel', data.revenueLearning.bestChannel],
    ['Revenue Learning Best Offer', data.revenueLearning.bestOffer],
    ['Best Industry', data.revenueLearning.bestIndustry],
    ['Calibration Status', data.revenueLearning.calibrationStatus],
    ...
    ['Studio Doctor Evidence Health', data.studioDoctor.evidenceHealth],
    ['Doctor Status', data.studioDoctor.doctorStatus],
    ['Repair Recommendations', String(data.studioDoctor.repairRecommendations)],
