...
import { buildMonthlySuccessReview } from './operatorRules';
...
function main(): void {
  ...
  const document = buildMonthlySuccessReview(input);
  fs.mkdirSync(outputRoot, { recursive: true });
  fs.writeFileSync(path.join(outputRoot, document.fileName), document.body, 'utf8');

  console.log(`Monthly success review generated: ${path.relative(process.cwd(), path.join(outputRoot, document.fileName))}`);
