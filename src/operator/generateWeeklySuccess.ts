...
import { buildWeeklySuccessReview } from './operatorRules';
...
function main(): void {
  ...
  const document = buildWeeklySuccessReview(input);
  fs.mkdirSync(outputRoot, { recursive: true });
  fs.writeFileSync(path.join(outputRoot, document.fileName), document.body, 'utf8');

  console.log(`Weekly success review generated: ${path.relative(process.cwd(), path.join(outputRoot, document.fileName))}`);
