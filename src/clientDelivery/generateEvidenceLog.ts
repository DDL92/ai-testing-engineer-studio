...
import { buildEvidenceDocument } from './clientDeliveryRules';
...
function main(): void {
  ...
  if (!id) {
    exitWithError('Missing required --id client_id argument. Example: npm run client:evidence -- --id demo-retainer-client');
  }
  ...
  console.log(`Evidence log generated: ${path.relative(process.cwd(), outputPath)}`);
  console.log(`Client: ${client.companyName}`);
  console.log('No evidence was invented. No APIs, browser automation, external services, credentials, or client systems were used.');
  console.log('Human approval is required before evidence is shared.');
}
