...
function main(): void {
  ...
  fs.writeFileSync(path.join(outputDir, 'lead-add-command-batch.md'), renderLeadAddCommandBatch(input, report), 'utf8');
  ...
  console.log(`Lead add command batch generated: ${relative(path.join(outputDir, 'lead-add-command-batch.md'))}`);
  ...
  console.log('Commands were generated only. No leads were added. No scraping, browsing, APIs, CRM, outreach automation, credentials, or external systems were used.');
}
