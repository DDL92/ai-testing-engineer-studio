...
import { buildClientPrepDocuments, eligibilityFailureReason, isEligibleForClientWorkflow } from './clientWorkflowRules';
...
function main(): void {
  ...
  if (!id) {
    exitWithError('Missing required --id lead_id argument. Example: npm run client:prep -- --id pushpress');
  }
  ...
  const documents = buildClientPrepDocuments(input);
  ...
  console.log(`Client prep generated: ${path.relative(process.cwd(), outputDir)}`);
  for (const document of documents) {
  ...
  console.log('Human approval is required before client communication.');
}
